...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    pesmiIzRacuna(polja.seznamRacunov, function(pesmi) {
      strankaIzRacuna(polja.seznamRacunov, function(strankaR) {
        if(!pesmi || !strankaR){
          odgovor.sendStatus(500);
        } else {
          console.log(strankaR[0]);
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    strankaIzID(zahteva.session.stranka, function(stranka) {
      if (!pesmi || !stranka) {
        odgovor.sendStatus(500);
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    try {
      ...
      stmt.finalize();
      uspesnaPrijava = "Stranka je bila uspešno registrirana.";
    } catch (err) {
      napaka2 = true;
      uspesnaPrijava = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
    }
...
streznik.get('/prijava', function(zahteva, odgovor) {
  vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        odgovor.render('prijava', {sporocilo: uspesnaPrijava, seznamStrank: stranke, seznamRacunov: racuni});  
      }) 
    });
})
